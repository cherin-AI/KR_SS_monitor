...
def _make_df(n: int = 10, market: str = "KOSPI") -> pd.DataFrame:
...
def test_meta_summary_keys():
    df_k = compute_score(_make_df(10, market="KOSPI"))
    df_q = compute_score(_make_df(8, market="KOSDAQ"))
    df_all = pd.concat([df_k, df_q], ignore_index=True)
    meta = build_meta_summary(df_all)
    for key in ["total_count", "kospi_count", "kosdaq_count", "avg_short_ratio", "avg_score",
...
def test_meta_high_pressure_count():
    df_k = compute_score(_make_df(10, market="KOSPI"))
    df_q = compute_score(_make_df(10, market="KOSDAQ"))
    df_all = pd.concat([df_k, df_q], ignore_index=True)
    meta = build_meta_summary(df_all, threshold=999.0)
